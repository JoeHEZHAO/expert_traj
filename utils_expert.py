...
class TrajectoryDataset(Dataset):
    ...
    def __init__(
        ...
        # angles = np.arange(0, 360, 45) if "test" not in self.data_dir else [0]
        # amplify = np.arange(1, 8, 0.5) if "test" not in self.data_dir else [0]
        # angles = [0]  # why would I have this?
            ...
            for angle in angles:
                ...
                for amp in amplify:
                    ...
                    for idx in range(0, num_sequences * self.skip + 1, skip):
                        ...
                        curr_seq_abs = np.zeros(
                            (len(peds_in_curr_seq), 2, self.seq_len)
                        )
                        curr_seq_norm = np.zeros(
                            (len(peds_in_curr_seq), 2, self.seq_len)
                        )
                        ...
                        for _, ped_id in enumerate(peds_in_curr_seq):
                            ...
                            a_curr_ped_seq = np.gradient(
                                # np.array(v_curr_ped_seq), 0.2, axis=1
                                np.array(v_curr_ped_seq),
                                # 0.4,
                                grad_eff,
                                # 0.2,
                                axis=1,
                            ...
                            # else:
                            # curr_ped_seq_norm[0, :] = curr_ped_seq_norm[0, :] - (x_mean_all)
                            # curr_ped_seq_norm[1, :] = curr_ped_seq_norm[1, :] - (y_mean_all)
        ...
        seq_list_start = np.concatenate(seq_list_start, axis=0)
        loss_mask_list = np.concatenate(loss_mask_list, axis=0)
        non_linear_ped = np.asarray(non_linear_ped)

        self.obs_traj_start = torch.from_numpy(seq_list_start).type(torch.float)
    ...
    def __getitem__(self, index):
        ...
        obs_start = self.obs_traj_start[start:end]
...
if __name__ == "__main__":
    ...
    data_set = os.path.join("./datasets", "univ", "test")
