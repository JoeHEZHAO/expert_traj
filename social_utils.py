...
def social_and_temporal_filter(
...
def collect_data(
    ...
    current_batch = []
    mask_batch = [
        [0 for i in range(int(batch_size * 1.5))] for j in range(int(batch_size * 1.5))
    ]
    ...
    for file in glob.glob(root_path + rel_path + part_file):
        ...
        while len(list(data_by_id.keys())) > 0:
            ...
            if current_size < batch_size:
                ...
                full_masks.append(
                    mask_batch[0 : len(current_batch), 0 : len(current_batch)]
                )
                ...
                current_batch = []
                mask_batch = [
                    [0 for i in range(int(batch_size * 1.5))]
                    for j in range(int(batch_size * 1.5))
                ]
            ...
            for i in range(1, len(curr_keys)):
                if social_and_temporal_filter(
                    curr_keys[0], curr_keys[i], all_data_dict, time_thresh, dist_tresh
                ):
...
def generate_pooled_data(
    b_size, t_tresh, d_tresh, train=True, scene=None, verbose=True
):
    if train:
    ...
    if not train:
        ...
        test_name = "./social_pool_data/test_{0}_{1}_{2}_{3}.pickle".format(
            "all" if scene is None else scene[:-2] + scene[-1], b_size, t_tresh, d_tresh
        )  # + str(b_size) + "_" + str(t_tresh) + "_" + str(d_tresh) + ".pickle"
...
class SocialDataset(data.Dataset):
    def __init__(
        ...
        "Initialization"
        load_name = "./social_pool_data/{0}_{1}{2}_{3}_{4}.pickle".format(
            set_name,
            "all_" if scene is None else scene[:-2] + scene[-1] + "_",
            b_size,
            t_tresh,
            d_tresh,
        )
        ...
        if id == False:
            for t in traj:
                t = np.array(t)
                t = t[:, :, 2:]  # Only use the last 2
                traj_new.append(t)

                if set_name == "train":
                    # augment training set with reversed tracklets...This is really important
        ...
        self.trajectory_ori = copy.deepcopy(traj_new)

        # self.trajectory_ori = np.copy(traj_new)
        self.trajectory_batches = np.copy(traj_new)
        self.mask_batches = masks_new.copy()
        self.initial_pos_batches = np.array(
            initial_pos(self.trajectory_batches)
